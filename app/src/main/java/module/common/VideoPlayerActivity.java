...
import media.VideoPlayerView;
import media.bannuku.BiliDanmukuDownloadUtil;
...
public class VideoPlayerActivity extends BaseActivity implements DanmukuSwitchListener, VideoBackListener {
    ...
    private void initMediaPlayer() {
        //配置播放器
        media.MediaController mediaController=new media.MediaController(this);
    ...
    public void loadData() {
        ...
        RetrofitHelper.getBiliGoAPI()
                ...
                .map(new Func1<HDVideoInfo, Uri>() {
                    @Override
                    public Uri call(HDVideoInfo hdVideoInfo) {
                        LogDog.i("播放地址url="+Uri.parse(hdVideoInfo.getDurl().get(0).getUrl()).toString());
                        return Uri.parse(hdVideoInfo.getDurl().get(0).getUrl());
                    }
                })
                ...
                .subscribe(baseDanmakuParser ->{
                    ...
                    mPrepareText.setText(startText);
                    LogDog.i("throwable="+throwable.toString());
                });
    }
